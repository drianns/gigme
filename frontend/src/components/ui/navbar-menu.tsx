    ...
    return (
        <Link href={href} className="flex space-x-2">
            ...
            <div>
                ...
                <p className="text-gray-600 text-sm max-w-[10rem] dark:text-neutral-300">
                    {description}
                </p>
            </div>
        </Link>
...
export const HoveredLink = ({ children, ...rest }: React.ComponentProps<typeof Link>) => {
    return (
        <Link
            {...rest}
            className="text-gray-600 dark:text-neutral-200 hover:text-emerald-600 transition-colors"
        >
            {children}
        </Link>
    );
